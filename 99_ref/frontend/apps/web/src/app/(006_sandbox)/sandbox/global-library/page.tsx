...
const TYPE_ICONS: Record<string, typeof Cloud> = {
  github: Github, // eslint-disable-line deprecation/deprecation
  aws: Cloud,
  azure: Cloud,
  gcp: Cloud,
  postgres: Database,
...
function DeployDialog({
  ...
  test: GlobalControlTestResponse | null
  onDeploy: (testId: string, orgId: string, workspaceId: string) => Promise<DeployResultResponse>
  onClose: () => void
}) {
  const { orgs, workspaces, selectedOrgId, selectedWorkspaceId } = useSandboxOrgWorkspace()
  const [orgId, setOrgId] = useState(selectedOrgId || "")
  const [wsId, setWsId] = useState(selectedWorkspaceId || "")
  ...
  useEffect(() => {
    if (test) {
      ...
      setWsId(selectedWorkspaceId || "")
      setDeploying(false)
      setResult(null)
      setError(null)
    }
  }, [test, selectedOrgId, selectedWorkspaceId])

  async function handleDeploy() {
    if (!test || !orgId || !wsId) return
    setDeploying(true)
    setError(null)
    try {
      const res = await onDeploy(test.id, orgId, wsId)
      setResult(res)
    } catch (e) {
  ...
  return (
    <Dialog open={!!test} onOpenChange={(v) => { if (!v) onClose() }}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle>Deploy to Workspace</DialogTitle>
              <DialogDescription>
                Deploy <span className="font-semibold">{test.name || test.global_code}</span> with all dependencies.
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
        ...
        {result ? (
          <div className="space-y-3">
            <div className="flex items-center gap-2 text-emerald-500">
              ...
              <span className="text-sm font-semibold">Deployed successfully</span>
            </div>
            <div className="rounded-lg border border-emerald-500/20 bg-emerald-500/5 p-3 space-y-1.5 text-xs">
              <p><span className="text-muted-foreground">Signals created:</span> {result.signal_count}</p>
              {result.created_threat_type_id && <p><span className="text-muted-foreground">Threat type:</span> created</p>}
              {result.created_policy_id && <p><span className="text-muted-foreground">Control test:</span> created</p>}
              {result.created_test_dataset_id && <p><span className="text-muted-foreground">Test dataset:</span> created</p>}
              {result.created_dataset_template_id && <p><span className="text-muted-foreground">Dataset template:</span> created</p>}
              <p className="text-muted-foreground pt-1">
                Source: {result.global_source_code} v{result.global_source_version}
              </p>
            </div>
            <DialogFooter>
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-lg border border-border/50 bg-muted/30 p-3 space-y-1.5">
              <span className="text-[10px] font-medium text-muted-foreground uppercase tracking-wide">Will deploy</span>
              <div className="flex flex-wrap gap-2 text-xs">
                <Badge variant="secondary" className="gap-1"><Zap className="h-3 w-3" />{test.signal_count} signal{test.signal_count !== 1 ? "s" : ""}</Badge>
                {test.bundle.threat_type && <Badge variant="secondary" className="gap-1"><Shield className="h-3 w-3" />Threat type</Badge>}
                {test.bundle.policy && <Badge variant="secondary" className="gap-1"><Globe className="h-3 w-3" />Control test</Badge>}
                {test.bundle.test_dataset && <Badge variant="secondary" className="gap-1"><TestTubes className="h-3 w-3" />{test.bundle.test_dataset.record_count} test records</Badge>}
                {test.bundle.dataset_template && <Badge variant="secondary" className="gap-1"><FileJson className="h-3 w-3" />Dataset template</Badge>}
              </div>
            </div>
            ...
            <DialogFooter className="gap-2 sm:gap-2">
              ...
              <Button size="sm" onClick={handleDeploy} disabled={deploying || !orgId || !wsId} className="gap-1.5">
                {deploying ? (
...
export default function GlobalLibraryPage() {
  ...
  const [deployTest, setDeployTest] = useState<GlobalControlTestResponse | null>(null)
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : "Failed to load global library")
    } finally {
      setLoading(false)
    }
  ...
  async function handleDeploy(testId: string, orgId: string, wsId: string): Promise<DeployResultResponse> {
    const result = await deployGlobalControlTest(testId, {
      org_id: orgId,
      workspace_id: wsId,
    })
    showSuccess(`Deployed ${result.global_source_code} — ${result.signal_count} signal(s), test dataset, and dependencies created`)
    await loadData()
    return result
  }
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Pre-built control test bundles ready to deploy. Each includes signals, threat types, test datasets, and policies.
            </p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-sm text-muted-foreground max-w-sm">
              Control test bundles will appear here once published by platform administrators.
            </p>
          </div>
        ) : (
          <div className="space-y-2">
            {tests.map((t) => {
              ...
              const hasTestDataset = !!t.bundle.test_dataset
              const hasTemplate = !!t.bundle.dataset_template
              return (
                <div
                  key={t.id}
                ...
                >
                  ...
                  <div className="flex items-center gap-4 px-4 py-3">
                    <button
                      ...
                      className="flex-1 text-left flex items-center gap-4 hover:bg-muted/20 transition-colors rounded-lg -m-1 p-1"
                    >
                        ...
                        <div className="flex items-center gap-3 mt-1.5">
                          ...
                          {t.bundle.policy && (
                            <div className="flex items-center gap-1 text-[10px] text-purple-500">
                              ...
                              {t.bundle.policy.actions.length} action{t.bundle.policy.actions.length !== 1 ? "s" : ""}
                            </div>
                          )}
                          {hasTestDataset && (
                    ...
                    {!isDeprecated && (
                      <Button
                        size="sm"
                        variant="outline"
                        className="gap-1.5 shrink-0 text-emerald-600 border-emerald-500/30 hover:bg-emerald-500/10 hover:text-emerald-600"
                        onClick={() => setDeployTest(t)}
                      >
                        <Download className="h-3.5 w-3.5" />
                        Deploy
                      </Button>
                    ...
                    <div className="px-4 pb-4 space-y-4 border-t border-border/30 pt-3">
                      ...
                      {t.bundle.dataset_template && (
                        <div className="rounded-lg border border-emerald-500/20 bg-emerald-500/[0.02] p-3 space-y-2">
                          ...
                          {t.bundle.dataset_template.sample_records.length > 0 && (
                            <div className="rounded-md bg-muted/50 border border-border/20 p-2 max-h-32 overflow-y-auto">
                              <span className="text-[9px] font-medium text-muted-foreground uppercase tracking-wide">
                                Sample Record
                              </span>
                              <pre className="text-[10px] font-mono text-foreground/80 whitespace-pre-wrap mt-1">
                                {JSON.stringify(t.bundle.dataset_template.sample_records[0], null, 2)}
                              </pre>
                            </div>
                          )}
                        </div>
                      ...
                      {!t.bundle.threat_type && !t.bundle.policy && !t.bundle.test_dataset && (
                        <p className="text-xs text-muted-foreground italic">
                          This bundle contains signal(s) only — no threat type, control test, or test dataset chain.
                        </p>
                      )}
                    </div>
                  )}
