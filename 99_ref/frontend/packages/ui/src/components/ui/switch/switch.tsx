...
const Switch = ({ children, className, ...props }: AriaSwitchProps) => (
  <AriaSwitch
    className={composeRenderProps(className, (className) =>
      cn(
        "group inline-flex items-center gap-2 text-sm font-medium leading-none transition-opacity data-[disabled]:cursor-not-allowed data-[disabled]:opacity-70",
        className
      )
    )}
    {...props}
      ...
      <>
        <div
          className={cn(
            ...
            /* Selected */
            "bg-[#4e5d72] group-data-[selected]:bg-[#4e5d72]",
            /* Readonly */
