"""
iam.dsar.delete.request — effect node.

Thin catalog-exposed wrapper around service.create_delete_request. Delegates all
...
class DsarDeleteRequest(_node_mod.Node):
    ...
    async def run(self, ctx: Any, inputs: "DsarDeleteRequest.Input") -> "DsarDeleteRequest.Output":
        pool = ctx.pool or (ctx.extras.get("pool") if ctx.extras else None)
        if pool is None:
            raise RuntimeError(
                "NodeContext.pool required for iam.dsar.delete.request "
