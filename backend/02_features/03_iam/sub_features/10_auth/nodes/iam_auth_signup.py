"""
iam.auth.signup — effect node.

Wraps the signup service: create email_password user + credential + session and
emit audit. Caller passes vault via NodeContext.extras['vault'].
"""
...
class AuthSignup(_node_mod.Node):
    key = "iam.auth.signup"
    kind = "effect"
    ...
    async def run(self, ctx: Any, inputs: "AuthSignup.Input") -> "AuthSignup.Output":
        ...
        if pool is None or vault is None:
            raise RuntimeError(
                "NodeContext.extras['pool'] + ['vault'] required for iam.auth.signup"
            )
