"""FastAPI routes for iam.magic_link."""
...
_schemas: Any = import_module(
    "backend.02_features.03_iam.sub_features.11_magic_link.schemas"
)
...
MagicLinkRequest = _schemas.MagicLinkRequest
MagicLinkConsume = _schemas.MagicLinkConsume

router = APIRouter(prefix="/v1/auth/magic-link", tags=["iam.magic_link"])
...
async def request_magic_link_route(body: MagicLinkRequest, request: Request) -> dict:
    ...
    async with pool.acquire() as conn:
        async with conn.transaction():
            ...
            await _service.request_magic_link(
                pool, conn, ctx,
    ...
    return _response.success({"sent": True, "message": "If that email is registered, a sign-in link is on its way."})


@router.post("/consume", status_code=200)
async def consume_magic_link_route(body: MagicLinkConsume, request: Request) -> Response:
    pool = request.app.state.pool
    vault = _vault(request)
    ctx_base = _build_ctx(request, pool)
    async with pool.acquire() as conn:
        async with conn.transaction():
            ctx = replace(ctx_base, conn=conn)
            session_token, user, session = await _service.consume_magic_link(
                pool, conn, ctx,
    ...
    if isinstance(exp, str):
        ...
        max_age = 7 * 24 * 3600
    _set_session_cookie(request, response, session_token, max_age)
    return response
