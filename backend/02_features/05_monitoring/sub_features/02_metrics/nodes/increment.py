"""monitoring.metrics.increment — append a counter increment.
...
class IncrementMetric(Node):
    key = "monitoring.metrics.increment"
    kind = "effect"
    emits_audit = True  # cardinality-reject failure emits; success hot-path bypass
    ...
    async def run(
        self, ctx: Any, inputs: "IncrementMetric.Input"
    ) -> "IncrementMetric.Output":
        conn = ctx.conn
        pool = ctx.extras.get("pool")
        req = _schemas.MetricIncrementRequest(
