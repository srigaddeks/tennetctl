...
Each sub-feature router owns its prefix (/v1/vault, /v1/vault-configs).
"""
...
_configs_routes: Any = import_module(
    "backend.02_features.02_vault.sub_features.02_configs.routes"
)

router = APIRouter()
router.include_router(_secrets_routes.router)
router.include_router(_configs_routes.router)
