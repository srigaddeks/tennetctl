...
import { useRotateSecret } from "@/features/vault/hooks/use-secrets";
import {
  secretRotateSchema,
...
export function RotateSecretDialog({
  ...
  const rotate = useRotateSecret(secretKey);
  ...
  async function onSubmit(values: SecretRotateForm) {
    ...
    try {
      ...
      revealRef.current = { key: secretKey, value: values.value };
    ...
    <>
      ...
      >
        <form
          ...
          data-testid={`rotate-secret-form-${secretKey}`}
        >
          <Field
            label="New value"
            required
            error={form.formState.errors.value?.message}
            htmlFor={`rotate-secret-value-${secretKey}`}
          >
            <Textarea
              id={`rotate-secret-value-${secretKey}`}
              placeholder="paste or type the new value"
              rows={4}
              autoFocus
              autoComplete="off"
              data-testid={`rotate-secret-value-${secretKey}`}
              {...form.register("value")}
            />
          </Field>
          ...
          {serverError && (
            <div
              ...
              data-testid={`rotate-secret-error-${secretKey}`}
            >
              {serverError}
            </div>
          )}
          ...
          <div className="mt-2 flex justify-end gap-2">
            ...
            <Button
              ...
              data-testid={`rotate-secret-submit-${secretKey}`}
            >
              Rotate
            </Button>
          </div>
