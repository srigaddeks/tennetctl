"""
TennetctlClient — somacrm's HTTP proxy to tennetctl.

Two auth modes:

* user_scoped: forwards user session bearer (used by get_me / whoami)
* system_scoped: uses somacrm's own service API key (Bearer nk_...) for
...
so audit failures never break the business flow.
"""
