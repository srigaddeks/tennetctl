"""
Tiny RBAC helper for somacrm.

Checks whether the caller has a given permission code (e.g.
'somacrm_contacts.create'). The auth scope (org/workspace) is already
enforced by middleware; this layer adds role-based gating on top.

Implementation: somacrm shares Postgres with tennetctl, so we query
