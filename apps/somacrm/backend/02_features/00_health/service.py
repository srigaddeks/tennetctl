"""Health service — pings tennetctl and reports somacrm uptime + version."""
...
_schemas = import_module("apps.somacrm.backend.02_features.00_health.schemas")

SOMACRM_VERSION = "0.1.0"
...
async def get_health(config, tennetctl_client, *, started_at_monotonic: float):
    ...
    return _schemas.HealthResponse(
        somacrm_version=SOMACRM_VERSION,
        somacrm_uptime_s=round(time.monotonic() - started_at_monotonic, 3),
