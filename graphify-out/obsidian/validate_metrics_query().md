---
source_file: "backend/02_features/05_monitoring/query_dsl/validator.py"
type: "code"
community: "Monitoring Query DSL"
location: "L58"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Monitoring_Query_DSL
---

# validate_metrics_query()

## Connections
- [[InvalidQueryError]] - `calls` [EXTRACTED]
- [[_depth_check()]] - `calls` [EXTRACTED]
- [[_validate_dsl()]] - `calls` [INFERRED]
- [[evaluate_rule()]] - `calls` [INFERRED]
- [[query()]] - `calls` [INFERRED]
- [[validator.py]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Monitoring_Query_DSL