---
source_file: "backend/02_features/03_iam/sub_features/13_passkeys/service.py"
type: "code"
community: "Session Auth & Middleware"
location: "L57"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Session_Auth_&_Middleware
---

# _b64url_decode()

## Connections
- [[auth_begin()]] - `calls` [EXTRACTED]
- [[auth_complete()]] - `calls` [EXTRACTED]
- [[parse_unsubscribe_token()]] - `calls` [EXTRACTED]
- [[register_begin()]] - `calls` [EXTRACTED]
- [[register_complete()]] - `calls` [EXTRACTED]
- [[service.py_1]] - `contains` [EXTRACTED]
- [[service.py_13]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Session_Auth_&_Middleware