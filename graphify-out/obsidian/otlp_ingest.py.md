---
source_file: "backend/02_features/05_monitoring/sub_features/01_logs/nodes/otlp_ingest.py"
type: "code"
community: "Admin Routes & DLQ"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Admin_Routes_&_DLQ
---

# otlp_ingest.py

## Connections
- [[Input_37]] - `contains` [EXTRACTED]
- [[OtlpLogsIngest]] - `contains` [EXTRACTED]
- [[Output_37]] - `contains` [EXTRACTED]
- [[monitoring.traces.otlp_ingest — effect node for OTLP traces ingest.]] - `rationale_for` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Admin_Routes_&_DLQ